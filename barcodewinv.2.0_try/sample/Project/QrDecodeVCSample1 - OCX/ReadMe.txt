========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : QrDecodeVCSample1
========================================================================


AppWizard が生成した QrDecodeVCSample1 には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには QrDecodeVCSample1 を構成している各ファイルの概要説明が含まれています。

QrDecodeVCSample1.dsp
...
QrDecodeVCSample1.h
    ...
    みます） をインクルードし、また CQrDecodeVCSample1App アプリケーションクラスを宣
    言します。

QrDecodeVCSample1.cpp
    このファイルは CQrDecodeVCSample1App アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。

QrDecodeVCSample1.rc
...
res\QrDecodeVCSample1.ico
    このファイルは、アプリケーションのアイコンとして使用されるアイコンファイ
    ルです。このアイコンはリソース ファイル QrDecodeVCSample1.rc によってインクルード
    されます。

res\QrDecodeVCSample1.rc2
...
QrDecodeVCSample1Dlg.h, QrDecodeVCSample1Dlg.cpp - ダイアログ
    これらのファイルは CQrDecodeVCSample1Dlg クラスを含みます。このクラスは 
    アプリケーションのメイン ダイアログの動作を定義します。ダイアログ
    テンプレートは Microsoft Visual C++ で編集可能な QrDecodeVCSample1.rc に含
    まれます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) QrDecodeVCSample1.pch 
